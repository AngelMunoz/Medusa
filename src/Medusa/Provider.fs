...
module Provider =
    ...
    let extractFilePath (uri: Uri) =
            ...
            if packagePath.StartsWith("@") then
                ...
                if firstSlash >= 0 then
                    ...
                    if secondSlash >= 0 then
                        ...
                        Error(InvalidPackagePath(packagePath, uri, "missing file path after scoped package@version"))
                else
                    Error(InvalidPackagePath(packagePath, uri, "missing package name separator in scoped package"))
            else
                ...
                if firstSlash >= 0 then
                    ...
                    Error(InvalidPackagePath(packagePath, uri, "missing file path after package@version"))
        ...
        | Error error ->
            let errorMsg =
                match error with
                | UnsupportedProvider(host, url) -> $"Unsupported URL provider '{host}' for URL '{url}'"
                | MissingPrefix(expectedPrefix, url) -> $"Unable to find '{expectedPrefix}' prefix in URL '{url}'"
